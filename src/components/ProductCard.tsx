...
export function ProductCard({ product }: { product: ProductProps }) {
  ...
  return (
    <div className="group bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden hover:shadow-xl transition-all flex flex-col">
      <Link href={`/produtos/${product.id}`} className="aspect-square bg-zinc-100 dark:bg-zinc-800 relative overflow-hidden block">
      ...
      <div className="p-5 flex-1 flex flex-col">
        ...
        <Link href={`/produtos/${product.id}`} className="mt-1 text-lg font-bold text-zinc-900 dark:text-zinc-50 group-hover:text-blue-600 transition-colors line-clamp-2">
          {product.name}
        </Link>
